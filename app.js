...
function saveOrder(data, result){
  ...
  con.query(sql, function(error, resultQuery){
    ...
    date = new Date()/1000;
    for(let i = 0; i < result.length; i++ ){
      sql = "insert into shop_order (date, user_id, goods_id, goods_cost, goods_amount, total) values ("+ date +","  + result[i]['id']+","+userId + "," + result[i]['cost']+","+ data.key[result[i]['id']]+","+ data.key[result[i]['id']]*result[i]['cost']+")";
      con.query(sql, function(error,resultQuery){
        if(error) throw error;
...
async function sendMail(data, result){
  ...
  let mailOption = {
    from : '<alsochi22@gmail.com>',
    to : 'alsochi22@gmail.com'+data.mail,
    subject: "Lite Shop order",
    text : "Ваш заказ в обработке",
    html: res
  };
